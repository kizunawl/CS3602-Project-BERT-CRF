...
class Example4Bert():
    ...
    def __init__(self, ex: dict):
        super(Example4Bert, self).__init__()
        self.ex = ex
        self.utt = ex['asr_1best']
        # self.utt = ex['manual_transcript']
        self.slot = {}
...
class SLUDataset(Dataset):
    def __init__(self, data_path) -> None:
        super().__init__()
        self.dataset = Example4Bert.load_dataset(data_path)
    
    def __getitem__(self, idx):
        return {'utt': self.dataset[idx].utt, 
                'labels': self.dataset[idx].tag_id}
