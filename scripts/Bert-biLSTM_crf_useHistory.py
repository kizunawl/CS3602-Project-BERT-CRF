...
from model.Bert_biLSTM_crf_useHistory import Model
...
if __name__ == "__main__":
    ...
    model = Model(args.num_tags, args.batch_size, Bert, args.finetune, args.hfactor, device).to(device)
    ...
    if not args.testing:
        ...
        for i in range(args.max_epoch):
            ...
            logger.info('Evaluation: \tEpoch: %d\tTime: %.4f\tDev acc: %.4f\tDev fscore(p/r/f): (%.4f/%.4f/%.4f)' % (i, time.time() - start_time, dev_acc, dev_fscore['precision'], dev_fscore['recall'], dev_fscore['fscore']))
            if dev_acc > best_result['dev_acc']:
                ...
                logger.info('NEW BEST MODEL: \tEpoch: %d\tDev loss: %.4f\tDev acc: %.4f\tDev fscore(p/r/f): (%.4f/%.4f/%.4f)' % (i, dev_loss, dev_acc, dev_fscore['precision'], dev_fscore['recall'], dev_fscore['fscore']))
        ...
        logger.info("Evaluation costs %.4fs ; Dev loss: %.4f\tDev acc: %.4f\tDev fscore(p/r/f): (%.4f/%.4f/%.4f)" % (time.time() - start_time, dev_loss, dev_acc, dev_fscore['precision'], dev_fscore['recall'], dev_fscore['fscore']))
